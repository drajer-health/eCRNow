...
| db-scheduler.polling-limit                           | Limit for polling tasks (if blank, default is used). |
| db-scheduler.table-name                              | Table name in the database to store scheduled tasks. |
| db-scheduler.immediate-execution-enabled=false       | Whether tasks should execute immediately after scheduling. |
| db-scheduler.scheduler-name=eCRNow-Instance-1        | Name for the scheduler instance (useful for multi-node deployments). |
| db-scheduler.threads=10                              | Number of threads to use for task execution. |
| db-scheduler.delay-startup-until-context-ready=false | Whether scheduler startup should wait until application context is fully ready. |
| timer.retries                                        | Number of retry attempts for timers in case of exceptions. |
| direct.tls.version=TLSv1.2                           | TLS version used for secure Direct Protocol communication. |
| scheduled.task.file.path                             | Path to scheduled task migration file. |
| cql.enabled=false                                    | Enable/disable CQL for PlanDefinition evaluation. |
| fhirpath.enabled=true                                | Enable/disable FHIRPath for PlanDefinition evaluation. |
| kar.directory'                                       | Directory containing ERSD and other PlanDefinition resources. |
| bsa.output.directory                                 | Directory to save BSA output files. |
| custom-query.directory                               | Directory where custom query files for PlanDefinitions are placed. |
| save.debug.files=true                                | Whether to save TriggerQuery, LoadingQuery, and eICR files for debugging. |
| enable.throttling=true                               | Enables EHR load control by throttling requests. |
| throttle.recheck.interval=5                          | Interval (in minutes) to recheck throttling conditions. |
| startup.timedelay=10                                 | Delay (in seconds) before accepting `launchPatient` requests. |
| ignore.timers=false                                  | For testing purposes in 3.0 only – ignore timers if set to true. |
| disable.hostname.verifier=true                       | Disables SSL hostname verification (use with caution, only in testing). |
| security.whitelist-endpoints                         | List of endpoints excluded from authentication (whitelisted). |
| socket.timeout=60                                    | Socket timeout in seconds. |
| connection.timeout=60                                | Connection timeout in seconds. |
| pool.max.per.route=60                                | Maximum connections per route. |
| pool.max.total=60                                    | Maximum total connections in the pool. |
| connection.request.time.out=60                       | Maximum time to wait for a connection from the pool. |
