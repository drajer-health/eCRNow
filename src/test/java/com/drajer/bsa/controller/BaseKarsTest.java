...
public class BaseKarsTest extends BaseIntegrationTest {
  ...
  public void testScenarioAndValidate() throws InterruptedException {
    ...
    try {
      ...
      Boolean reportBundleGenerated =
          this.reportBundleGenerated(this.testCaseInfo.getName(), this.testCaseInfo.getPlanDefUrl());
  ...
  protected Boolean reportBundleGenerated(String patientId, String planDefUrl) {
    ...
    Boolean submitted = validateActionStatus(submittedActionId, BsaTypes.BsaActionStatusType.Completed);
    String routeAndSendActionId = String.format("%s-PlanDefinition/%s", "route-and-send-eicr", planDefUrl);
    Boolean sent = validateActionStatus(routeAndSendActionId, BsaTypes.BsaActionStatusType.Completed);
