...
public class TestFhirPathEvaluator {
    ...
    public void testEvaluateFhirPathUsingBundleWhereAndSelect() {
        String bundleExpression = "Bundle.entry.select(resource as Patient).where(gender='female').select(name.family)";
    ...
    public void testEvaluateFhirPathUsingBundleSelectAndWhere() {
        String bundleExpression = "Bundle.entry.resource.ofType(Patient).where(gender='female').select(name.family)";
        Bundle bundle = createBundle();
        List<IBase> results = FhirPathEvaluator.evaluateFhirPath(bundle, bundleExpression);
        String family = results.get(0).toString();
        Assertions.assertEquals("Smith", family);
    ...
    public void testEvaluateFhirPathUsingBundleComplex() {
        String bundleExpression = "Bundle.entry.select(resource as Patient).first().where(gender='female').exists()";
        Bundle bundle = createBundle();
        List<IBase> results = FhirPathEvaluator.evaluateFhirPath(bundle, bundleExpression);
    ...
    public void testEvaluateFhirPathUsingBundleComplex1() {
        String bundleExpression = "(Bundle.entry.resource.first() is Patient and Bundle.entry.resource[1].name.exists())" +
