...
public class RrReceiverImplTest {
  ...
  public void testHandleFailureMdn_Success() {
    ...
    PublicHealthMessage localPhm = new PublicHealthMessage();
    when(phDao.getByCorrelationId(xCorrelationId)).thenReturn(localPhm);

    rrReceiver.handleFailureMdn(data, xCorrelationId, xRequestId);

    assertEquals(EicrTypes.RrType.FAILURE_MDN.toString(), localPhm.getResponseMessageType());
    assertEquals("<xml>test</xml>", localPhm.getFailureResponseData());
    assertNull(localPhm.getCdaResponseData());
    assertNull(localPhm.getResponseDataId());
    assertNull(localPhm.getResponseReceivedTime());
    assertNull(localPhm.getResponseProcessingInstruction());
    assertNull(localPhm.getResponseProcessingStatus());
    assertNull(localPhm.getResponseEhrDocRefId());
    verify(phDao, times(1)).saveOrUpdate(localPhm);
  }
  ...
  public void testHandleReportabilityResponse_Success() {
    ...
    CdaRrModel localRrModel = mock(CdaRrModel.class);
    CdaIi rrDocId = mock(CdaIi.class);
    CdaIi eicrDocId = mock(CdaIi.class);

    when(rrParser.parse(data.getRrXml())).thenReturn(localRrModel);
    when(localRrModel.getRrDocId()).thenReturn(rrDocId);
    when(localRrModel.getEicrDocId()).thenReturn(eicrDocId);
    when(rrDocId.getRootValue()).thenReturn("rr-doc-id");
    when(eicrDocId.getRootValue()).thenReturn("eicr-doc-id");

    PublicHealthMessage localPhm = new PublicHealthMessage();
    when(phDao.getBySubmittedDataId(eicrDocId.getRootValue())).thenReturn(localPhm);

    HealthcareSetting localHs = new HealthcareSetting();
    localHs.setCreateDocRefForResponse(true);
    localHs.setHandOffResponseToRestApi("https");
    when(hsDao.getHealthcareSettingByUrl(localPhm.getFhirServerBaseUrl())).thenReturn(localHs);

    rrReceiver.handleReportabilityResponse(data, xRequestId);

    assertEquals(
        EicrTypes.RrType.REPORTABILITY_RESPONSE.toString(), localPhm.getResponseMessageType());
    assertEquals("<xml>test</xml>", localPhm.getCdaResponseData());
    assertEquals("rr-doc-id", localPhm.getResponseDataId());
    assertNotNull(localPhm.getResponseReceivedTime());
    verify(phDao, times(1)).saveOrUpdate(localPhm);
  }
  ...
  public void testHandleReportabilityResponse_HealthcareSettingNotFound1() {
    ...
    when(hsDao.getHealthcareSettingByUrl(localPhm.getFhirServerBaseUrl())).thenReturn(null);

    rrReceiver.handleReportabilityResponse(data, xRequestId);

    assertEquals(
        EicrTypes.RrProcessingStatus.HEALTHCARE_SETTING_NOT_FOUND_FOR_RR.toString(),
        localPhm.getResponseProcessingStatus());

    assertEquals("rr-doc-id", localPhm.getResponseDataId());
    assertNotNull(localPhm.getResponseProcessingStatus());
  }
  ...
  public void testHandleReportabilityResponse_EmptyRrXml() {
    ...
    ch.qos.logback.classic.Logger localLogger =
  ...
  public void testPostDocRefToEhr_FailurePosting() throws JSONException {
    ...
    MethodOutcome localMethodOutcome = new MethodOutcome();
    localMethodOutcome.setCreated(false);
    when(fhirContextInitializer.submitResource(any(), eq(docRef))).thenReturn(localMethodOutcome);

    rrReceiver.postDocRefToEhr(docRef, phm, hs, rrModel);
  }

  @Test
  public void testPostDocRefToEhr_Success() throws JSONException {
  ...
  public void testConstructDocumentReference_Success1() {
    PublicHealthMessage localPhm = new PublicHealthMessage();
    localPhm.setResponseProcessingInstruction(EicrTypes.ReportabilityType.RRVS1.toString());
    localPhm.setPatientId("patient123");
    localPhm.setEncounterId("encounter123");

    HealthcareSetting localHs = new HealthcareSetting();
    localHs.setDefaultProviderId("provider123");
    localHs.setDocRefMimeType("application/xml");

    CdaRrModel localRrModel = new CdaRrModel();
    ...
    when(ehrService.constructR4DocumentReference(
            rrXml,
            localPhm.getPatientId(),
            localPhm.getEncounterId(),
            localHs.getDefaultProviderId(),
            localHs.getDocRefMimeType(),
    ...
    DocumentReference result =
        rrReceiver.constructDocumentReference(localPhm, localRrModel, localHs, rrXml);
  ...
  public void testConstructDocumentReference_NullResult1() {
    PublicHealthMessage localPhm = new PublicHealthMessage();
    localPhm.setResponseProcessingInstruction("NonMatchingInstruction");
