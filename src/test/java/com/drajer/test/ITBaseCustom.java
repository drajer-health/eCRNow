...
public class ITBaseCustom extends BaseIntegrationTest {
  ...
  private void saveHealtcareSetting(String healthCareSettingsFile) throws IOException {
    ...
    // healthcareSetting = mapper.readValue(response.getBody(), HealthcareSetting.class);
    int res = response.getStatusCode().value();
    System.out.println(response.getStatusCode());

    // -----------------------------------
    ResponseEntity<String> response1 =
        restTemplate.getForEntity(createURLWithPort("/api/healthcareSettings/"), String.class);

    int res1 = response1.getStatusCode().value();
