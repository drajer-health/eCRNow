...
public class Dstu2ResourcesData {
	...
	public Encounter getEncounterData(FhirContext context, IGenericClient client, LaunchDetails launchDetails,
			Dstu2FhirData dstu2FhirData, Date start, Date end) {
		...
		if (!encounter.getId().getValue().isEmpty() && encounter != null) {
			for (Entry entry : bundle.getEntry()) {
				...
				if (!condition.getEncounter().isEmpty()) {
					if (condition.getEncounter().getReference().getIdPart()
	...
	public List<Observation> getPregnancyObservationData(FhirContext context, IGenericClient client,
			LaunchDetails launchDetails, Dstu2FhirData dstu2FhirData, Encounter encounter, Date start, Date end) {
		Bundle bundle = (Bundle) resourceData.getObservationByPatientIdAndCode(launchDetails, client, context,
	...
	public List<Observation> getTravelObservationData(FhirContext context, IGenericClient client,
			LaunchDetails launchDetails, Dstu2FhirData dstu2FhirData, Encounter encounter, Date start, Date end) {
		Bundle bundle = (Bundle) resourceData.getObservationByPatientIdAndCode(launchDetails, client, context,
				"Observation", QueryConstants.TRAVEL_CODE, QueryConstants.LOINC_CODE_SYSTEM);
		List<Observation> observations = new ArrayList<>();
		observations = getObservations(observations, bundle, encounter, start, end);
	...
	public Medication getMedicationData(FhirContext context, IGenericClient client, LaunchDetails launchDetails,
			Dstu2FhirData dstu2FhirData, String medicationId) {
		...
		if (bundle != null && !encounter.getId().getValue().isEmpty() && encounter != null) {
			for (Entry entry : bundle.getEntry()) {
				...
				if (!medAdministration.getEncounter().isEmpty()) {
					if (medAdministration.getEncounter().getReference().getIdPart()
							.equals(encounter.getIdElement().getIdPart())) {
	...
	public List<DiagnosticOrder> getDiagnosticOrderData(FhirContext context, IGenericClient client,
			LaunchDetails launchDetails, Dstu2FhirData dstu2FhirData, Encounter encounter, Date start, Date end) {
		Bundle bundle = (Bundle) resourceData.getResourceByPatientId(launchDetails, client, context, "DiagnosticOrder");
		...
		if (!encounter.getId().getValue().isEmpty() && encounter != null) {
			for (Entry entry : bundle.getEntry()) {
				...
				if (!diagnosticOrder.getEncounter().isEmpty()) {
					if (diagnosticOrder.getEncounter().getReference().getIdPart()
							.equals(encounter.getIdElement().getIdPart())) {
		...
		if (!encounter.getId().getValue().isEmpty() && encounter != null) {
			for (Entry entry : bundle.getEntry()) {
				...
				if (!immunization.getEncounter().isEmpty()) {
					if (immunization.getEncounter().getReference().getIdPart()
							.equals(encounter.getIdElement().getIdPart())) {
		...
		if (!encounter.getId().getValue().isEmpty() && encounter != null) {
			for (Entry entry : bundle.getEntry()) {
				...
				if (!diagnosticReport.getEncounter().isEmpty()) {
					if (diagnosticReport.getEncounter().getReference().getIdPart()
							.equals(encounter.getIdElement().getIdPart())) {
