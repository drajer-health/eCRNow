	...
	public static String generateImmunizationSection(R4FhirData data, LaunchDetails details) {
		...
		if(imms != null && imms.size() > 0) {		
	        ...
	        sb.append(CdaGeneratorUtils.getXmlForText(CdaGeneratorConstants.TITLE_EL_NAME, 
	            CdaGeneratorConstants.IMMUNIZATION_SEC_TITLE));
            ...
            for (Immunization imm : imms)
            {
                ...
                if(imm.getOccurrenceDateTimeType() != null) {
                	sb.append(CdaGeneratorUtils.getXmlForEffectiveTime(CdaGeneratorConstants.EFF_TIME_EL_NAME, imm.getOccurrenceDateTimeType().toString()));
                }
                else {
