...
public class CdaSocialHistoryGenerator {
	...
	public static String generateSocialHistorySection(R4FhirData data, LaunchDetails details) {
		...
		if(birthSex != null || 
			...
			if(pregObs != null && pregObs.size() > 0) {
				...
				// These are not available in FHIR right now reliably, so nothing to process until further discussion with vendors.
	...
	public static String generateBirthSexEntry(R4FhirData data, LaunchDetails details, CodeType birthSex) {
        ...
        sb.append(CdaGeneratorUtils.getXmlForTemplateId(CdaGeneratorConstants.BIRTH_SEX_OBS_TEMPLATE_ID, CdaGeneratorConstants.BIRTH_SEX_OBS_TEMPLATE_ID_EXT));
