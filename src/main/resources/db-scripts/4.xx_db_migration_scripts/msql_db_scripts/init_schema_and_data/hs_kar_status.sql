...
IF NOT EXISTS (
    ...
    WHERE name = 'hs_kar_status_v2_SEQ' AND SCHEMA_ID = SCHEMA_ID('dbo')
)
BEGIN
    CREATE SEQUENCE dbo.hs_kar_status_v2_SEQ
        AS INT
        START WITH 1
        INCREMENT BY 50
        NO MINVALUE
    ...
    BEGIN
        ...
        SET @sql = N'
            CREATE TABLE dbo.' + QUOTENAME(@new_table) + N' (
                id INT DEFAULT NEXT VALUE FOR dbo.hs_kar_status_v2_SEQ PRIMARY KEY,
                hs_id INT NOT NULL,
                hs_fk INT NULL,
                ...
                last_activation_date DATETIME2(6) NULL,
                last_inactivation_date DATETIME2(6)  NULL,
            ...
            CREATE INDEX idx_hs_id_new
            ON dbo.' + QUOTENAME(@new_table) + N' (hs_id);
        ';
